...
\end{figure}
\footnotetext{Best result from the MS MARCO leaderboard or the corresponding paper.}
}
...
\begin{figure}
\includegraphics[width=\textwidth]{Grafiken/questions} 
\end{figure}
\vspace{-4cm}
\begin{center}
  \begin{LARGE}
...
where \text{Precision@k} represents the precision at position k and \text{Relevance@k} represents the binary relevance label (1 if relevant, 0 if non-relevant) at position k. \\[1cm]
$\Rightarrow$ In context of this paper rankings are based on binarized judgments where four-point scale from nDCG@10 is used: Only labels of 2 and 3 are treated as relevant.
}
